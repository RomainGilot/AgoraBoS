...
class PdoJeux
{
    ...
    public function getLesEquipements(): array
    {
        $requete =  'SELECT ref_equipement_WH as identifiant, nom_WH as libelle, nb_place_WH, te.libelle_WH
						FROM equipement INNER JOIN type_equipement ON type_equipement_WH = code_TypeEquipement_WH
						ORDER BY nom_WH';
        try {
            $resultat = PdoJeux::$monPdo->query($requete);
    ...
    public function ajouterEquipement(string $refEquipement_WH, string $libEquipement_WH, int $nbPlace_WH, string $typeEquipement_WH)
    {
        try {
            $requete_prepare = PdoJeux::$monPdo->prepare("INSERT INTO equipement "
                . "(ref_equipement_WH, nom_WH, nb_place_WH, type_equipement_WH) "
                . "VALUES (:unRefEquipement_WH, :unLibEquipement_WH, :nbPlace_WH, :unTypeEquipement_WH) ");
                $requete_prepare->bindParam(':unRefEquipement_WH', $refEquipement_WH, PDO::PARAM_STR);
                $requete_prepare->bindParam(':unLibEquipement_WH', $libEquipement_WH, PDO::PARAM_STR);
                $requete_prepare->bindParam(':unTypeEquipement_WH', $typeEquipement_WH, PDO::PARAM_STR);
                $requete_prepare->bindParam(':nbPlace_WH', $nbPlace_WH, PDO::PARAM_INT);
            $requete_prepare->execute();
            // récupérer l'identifiant crée
            return PdoJeux::$monPdo->lastInsertId();
        } catch (Exception $e) {
    ...
    public function modifierEquipement(string $refEquipement_WH, string $libEquipement_WH, int $nbPlace_WH)
    {
        try {
            $requete_prepare = PdoJeux::$monPdo->prepare("UPDATE equipement "
                . "SET nom_WH = :unLibEquipement_WH, nb_place_WH = :nbPlace_WH, type_equipement_WH = :unTypeEquipement_WH "
                . "WHERE equipement.ref_equipement_WH = :unRefEquipement_WH");
                $requete_prepare->bindParam(':unRefEquipement_WH', $refEquipement_WH, PDO::PARAM_STR);
                $requete_prepare->bindParam(':unLibEquipement_WH', $libEquipement_WH, PDO::PARAM_STR);
                $requete_prepare->bindParam(':unTypeEquipement_WH', $typeEquipement_WH, PDO::PARAM_STR);
                $requete_prepare->bindParam(':nbPlace_WH', $nbPlace_WH, PDO::PARAM_INT);
    ...
    public function supprimerEquipement(int $idEquipement_WH): void
    {
        try {
            $requete_prepare = PdoJeux::$monPdo->prepare("DELETE FROM equipement "
                . "WHERE equipement.ref_equipement_WH = :unIdEquipement_WH");
            $requete_prepare->bindParam(':unIdEquipement_WH', $idEquipement_WH, PDO::PARAM_INT);
            $requete_prepare->execute();
        } catch (Exception $e) {
            die('<div class = "erreur">Erreur dans la requête !<p>'
                . $e->getmessage() . '</p></div>');
